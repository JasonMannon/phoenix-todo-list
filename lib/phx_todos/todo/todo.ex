defmodule PhxTodos.Todo do
  ...
  # Custom filter action that returns todo_items with a certain status
  def filter_todo_items(status) do
    from(t in TodoItem, where: t.status == ^status)
    |> Repo.all()
  end
end
